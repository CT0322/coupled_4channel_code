...
//read perambulaors_c
	...
	if(quarks[i].type == "c"){
		...
		sprintf(infile, "%s/cnfg%04d/rnd_vec_%02d/perambulator.rndvecnb%02d.c.Tso%s%04d.Vso%s%04d.DsoF4.TsiF%04d.SsiF%d.DsiF4.CsiF3.smeared0.%05d", filename.c_str(),config_i, rnd_vec_0i, rnd_vec_0i, &quarks[i].dilution_T[1], number_of_dilution_T, &quarks[i].dilution_E[1], number_of_dilution_E, Lt, Lx*Ly*Lz, config_i);
...
// read perambulaotrs_u
      ...
      sprintf(infile,
          "%s/cnfg%04d/rnd_vec_%02d/perambulator.rndvecnb%02d.u.Tso%s%04d.Vso%s%04d.DsoF4.TsiF%04d."
          "SsiF%d.DsiF4.CsiF3.smeared0.%05d", filename.c_str(),config_i, rnd_vec_0i, rnd_vec_0i, &quarks[i].dilution_T[1],
          number_of_dilution_T, &quarks[i].dilution_E[1], number_of_dilution_E, Lt, Lx*Ly*Lz, config_i);
...
//read rnd_vec_c
    ...
    for(int rnd_vec_0i = 0; rnd_vec_0i < number_of_rnd_vec; ++rnd_vec_0i){
      std::string filename = global_data->get_path_perambulators_c();
      sprintf(infile, "%s/cnfg%04d/rnd_vec_%02d/randomvector.rndvecnb%02d.c.nbev%04d.%04d", 
...
//read rnd_vec_u
       ...
       for(int rnd_vec_0i = 0; rnd_vec_0i < number_of_rnd_vec; ++rnd_vec_0i){
       std::string filename = global_data->get_path_perambulators_u();
       sprintf(infile, "%s/cnfg%04d/rnd_vec_%02d/randomvector.rndvecnb%02d.u.nbev%04d.%04d",
          filename.c_str(), config_i, rnd_vec_0i, rnd_vec_0i, number_of_eigen_vec, config_i);
